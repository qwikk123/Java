...
public class StringJustify {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 2) {System.out.println("only 2 arg \"string\" \"length\"");System.exit(0);}
        int length = 0;
        ...
        for (int i = 0; i<tokens.length; i++) {
            ...
            if (lineLength+i-rest>length) {
                ...
                if (i-rest-1 < 1) {spaces--;}
                Arrays.fill(space, " ");

                int adder = 0;
                while (spaces > 0) {
                    if (adder > space.length-1) {adder = 0;}
                    space[adder]+= " ";
                    adder++;
                    spaces--;
                }
                ...
                for (int x = rest; x < i; x++) {
                    line += tokens[x];
                    if (addspace < space.length) {line += space[addspace];}
                    addspace++;
                }
                printout+=line+"\n";
